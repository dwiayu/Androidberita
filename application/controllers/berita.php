...
require APPPATH . 'libraries/Format.php';
    ...
    class berita extends REST_Controller{
        ...
        function user_get(){
            ...
            SELECT
            ber.id_berita, ber.id_sumber, ber.id_kategori, ber.judul, ber.isi from berita ber, sumber, kategori
            WHERE ber.id_sumber=sumber.id_sumber AND ber.id_kategori=kategori.id_kategori")->result();
                $this->response(
                    array(
                        "status"=> "success", 
        ...
        // }
        function user_post($data_berita){
            $data_berita = array(
    ...
    function user_delete($data_berita){
        $data_berita = $this->delete('id_berita');
        $getId= $this->db->query("SELECT id_berita from berita WHERE id_berita='".$data_berita['id_berita']."'")->result();
        if(empty($getId)){
            $this->response(array('status'=>'fail',"message"=>"id_berita tidak ada/salah"));
        }else{
